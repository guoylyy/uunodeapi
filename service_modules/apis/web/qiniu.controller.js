...
pub.fetchQiniuUploadToken = (req, res) => {
  return requestUtil.compactFileNamePromise(req.body)
      ...
      .then((tokenBody) => {
        debug(tokenBody);
        req.__MODULE_LOGGER('获取七牛上传文件token', tokenBody);

        let qiniuTokenObj = qiniuComponent.fileTokenBodyHandler(tokenBody, systemConfig.WEB_BASE_QINIU_CALLBACK_URL, req.__CURRENT_USER.id);
...
pub.qiniuCallbackHandler = (req, res) => {
  ...
  if (!qiniuComponent.isCallbackValid(systemConfig.WEB_BASE_QINIU_CALLBACK_URL, callbackBody, auth)) {
